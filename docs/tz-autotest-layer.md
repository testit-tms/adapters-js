...
## JavaScript adapters status

Implemented in `testit-js-commons` (shared by all JS adapters):

- `TestLayers` constants (`testit-js-commons` export)
- `AutotestPost.layer`, `Metadata.layer`
- `AutotestConverter`: `layer: { name, source: Run }` on create when set; on update always `resetLayer: false`, `layer` only when set
- `AutotestsService.updateAutotestFromFailed`: applies `layer` from test when present

### Syntax by adapter

| Adapter | How to declare layer in test code |
|---------|-----------------------------------|
| Jest | `testit.layer('API')` or `testit.layer(TestLayers.API)` |
| Mocha | `this.layer = 'API'` in test / hook |
| Playwright | `await testit.layer('API')` |
| Cypress | `tms.addLayer('API')` via `getTestRuntime()` |
| Codecept | `{ layer: 'API' }` in scenario opts |
| Cucumber | `@Layer=API` on scenario / feature |
| TestCafe | `layer: 'API'` in test metadata object |
