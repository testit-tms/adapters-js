...
import { isTmsHook, isRootAfterAllHook, isTestReported } from "../utils.js";

export const TMS_REPORT_SYSTEM_HOOK = "__tms_report_system_hook__";
...
export const injectFlushMessageHooks = () => {
  afterEach(TMS_REPORT_SYSTEM_HOOK, flushRuntimeMessages);
  after(TMS_REPORT_SYSTEM_HOOK, onAfterAll);
};
...
const onFail = (testOrHook: CypressTest | CypressHook, err: Error) => {
  ...
  const isTmsHookFailure = isHook && isTmsHook(testOrHook);
