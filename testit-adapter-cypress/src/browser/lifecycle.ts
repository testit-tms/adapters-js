...
export const flushRuntimeMessages = () => getTestRuntime().flushTmsMessagesToTask("reportTmsCypressSpecMessages");

export const completeSpecAsync = () =>
  getTestRuntime().flushTmsMessagesToTaskAsync("reportFinalTmsCypressSpecMessages");

const reportCurrentTestIfAny = () => {
  const currentTest = getCurrentTest();
  if (currentTest) {
    reportTestEnd(currentTest);
    markTestAsReported(currentTest);
  }
};
...
const logTmsRootAfterError = (context: Mocha.Context, err: unknown) => {
